...
createBlogModal
  ...
  .addEventListener("click", closeCreateBlogModal);
createBlogModal
...
async function loadBlogs() {
  ...
  snapshot.forEach((docSnap) => {
    ...
    col.innerHTML = `
  <div class="card blog-card" data-id="${docSnap.id}">
    <div class="card-content">
      <h3 class="title is-5">${blog.title}</h3>
      <p class="content">${blog.content.substring(0, 120)}...</p>
      <p class="is-size-7 has-text-grey">
        Published: ${new Date(blog.createdAt.toMillis()).toLocaleString()}
      </p>
    </div>
  </div>
`;
    col.querySelector(".card").addEventListener("click", () => {
      openBlogViewModal(blog);
    });
    ...
    const viewBlogImageWrapper = document.getElementById(
      "viewBlogImageWrapper"
    );
    const viewBlogImage = document.getElementById("viewBlogImage");
    const viewBlogDate = document.getElementById("viewBlogDate");

    function openBlogViewModal(blog) {
      viewBlogTitle.textContent = blog.title;
      viewBlogContent.textContent = blog.content;
      viewBlogDate.textContent = `Published: ${new Date(
      ...
      if (blog.imageUrl) {
        ...
        viewBlogImageWrapper.style.display = "block";
      } else {
        viewBlogImageWrapper.style.display = "none";
      }

      viewBlogModal.classList.add("is-active");
    }

    function closeBlogViewModal() {
      viewBlogModal.classList.remove("is-active");
    }

    viewBlogModal
      .querySelector(".modal-background")
      .addEventListener("click", closeBlogViewModal);
    viewBlogModal
      .querySelector(".delete")
      .addEventListener("click", closeBlogViewModal);
...
async function loadAdminBlogs() {
  ...
  snapshot.forEach((docSnap) => {
    ...
    item.innerHTML = `
      ...
      <button class="button is-small is-info mt-2" data-edit="${
        docSnap.id
      }">Edit</button>
      <button class="button is-small is-danger mt-2" data-delete="${
        docSnap.id
      }">Delete</button>
    `;
